...
**Seit 07.08.2026 läuft derselbe Prüflauf zusätzlich als GitHub-Action** bei jedem Pull Request
gegen `main` (`.github/workflows/prueflauf.yml`). Das ersetzt den Lauf von Hand **nicht** – wer
lokal committet, prüft weiter selbst –, aber ein vergessener Prüflauf kann nicht mehr unbemerkt
live gehen. Bewusst **kein** Lauf auf `push` nach `main` – dort hat der Deploy-Webhook längst
ausgeliefert, ein Fehlschlag käme zu spät.

Die Action checkt das **Backend-Repo mit aus** (beide Repos sind öffentlich, es braucht kein
Secret) und setzt `KEPLER_BACKEND_SERVER`. Ohne das fehlt genau die Frontend-/Backend-Parität:
`test_flottenbalance`, `test_kampfphasen`, `test_konter_paritaet` und `test_paritaet_tabellen`
überspringen sich mit Exit-Code 0 – und **schlimmer**, `test_angriffssumme`, `test_hilfe_effekte`
und `test_pvp_vorschau` prüfen ihre Backend-Vergleiche hinter einem `if (fs.existsSync(...))`
einfach nicht mehr, **ohne** ein SKIP zu melden. Der grüne Haken sähe identisch aus und verschwiege
sieben Prüfungen. (Sechs weitere Dateien importieren `SERVER_JS`, ohne es zu benutzen – toter
Rest aus früheren Fassungen, harmlos, aber beim nächsten Anfassen entfernen.)
...
3. **Boot+Tab-Sweep im echten Browser**: `tests/sweep.js` – Spieldatei in Chromium laden (Playwright), alle Tab-Buttons durchklicken, auf Konsolenfehler prüfen. Mit realistischem Spielstand (mehrere Kolonien inkl. Mond, aktive Forschung/Missionen) – ein leerer Spielstand übersieht Bugs, die erst bei voller Array-Traversierung auftreten. Läuft als Teil von `node tests/run.js` automatisch mit, ein eigener Aufruf ist nicht nötig. **Der Sweep hat eine bewusste Grenze**: Seine Backend-Attrappe antwortet mager (`return j({})`), prüft also den Solo-/Offline-Pfad. Anzeigebereiche, die ohne Serverdaten in einen Leerzustand gehen, führen ihren Rendercode dabei nie aus – genau darin steckte der Fraktionsladen-Fehler monatelang (wo nichts rendert, kann nichts krachen). Die Gegenprobe MIT gefüllten Serverdaten ist `tests/test_serverbereiche.js`; beide Tests decken verschiedene Zustände ab und sollen nicht zusammengelegt werden. *(Hier stand bis 07.08.2026 „JSDOM-Boot+Tab-Sweep … Datei in jsdom laden, `runScripts:'dangerously'`". Das beschrieb einen Stand, den es nicht mehr gab: 126 der 143 Tests laufen längst über Playwright mit echtem Chromium, genau **eine** Datei erwähnt jsdom überhaupt noch. Ein Musterfall der eigenen Regel 6 – die zweite Anzeigestelle, die die alte Annahme behielt, war diesmal die Anleitung selbst.)*
...
**PRs sofort mergen**: Offene PRs nach dem Push ohne Rückfrage direkt mergen (nicht als Draft offen lassen) – sonst landen Änderungen nicht auf `main`. Gilt für Frontend- und Backend-Repo gleichermaßen. **Seit der Webhook bekannt ist, wiegt das schwerer als gedacht**: Der Merge ist nicht bloß ein Zwischenschritt zu einem späteren manuellen Deploy, sondern die Auslieferung selbst – was gemerged wird, steht Sekunden später auf `gamegeeeeek.de`. Der Prüflauf (`node tests/run.js`, grün) ist deshalb keine Formalie, sondern das letzte, was zwischen einer Änderung und den Spielern steht. Seit 07.08.2026 läuft er zusätzlich als GitHub-Action am Pull Request (`.github/workflows/prueflauf.yml`) – **vor** dem Merge und damit vor der Auslieferung. Ein roter Lauf dort ist der einzige Grund, einen PR nicht sofort zu mergen.
